...
public class ResourceExtractor {
    ...
    public List<ExtractedResource> extract(byte[] packageBytes, String resourceType) throws IOException {
        ...
        try (InputStream bis = new ByteArrayInputStream(packageBytes);
            ...
            while ((entry = tais.getNextEntry()) != null) {
                if (!isMatchingResourceEntry(entry, resourceType)) continue;
                try {
    ...
    private static boolean isMatchingResourceEntry(TarArchiveEntry entry, String resourceType) {
        return !entry.isDirectory() &&
                entry.getName().startsWith("package/" + resourceType) &&
