 ...
 * Elimina duplicação de código entre LocalPackageSource e
...
public class PackageJsonReader {
    ...
    public static PackageMetadata readMetadata(byte[] packageBytes) throws IOException {
        return readFromPackageJson(packageBytes, PackageJsonReader::parseMetadata);
    }
    ...
    public static PackageBasicInfo readBasicInfo(byte[] packageBytes) throws IOException {
        return readFromPackageJson(packageBytes, PackageJsonReader::parseBasicInfo);
    }

    /**
     * Parseia metadados básicos diretamente de uma String JSON.
     * Útil para testes ou quando o JSON já foi extraído.
     */
    public static PackageBasicInfo parseBasicInfo(String jsonContent) {
        try {
            ...
            throw new RuntimeException("Erro ao parsear JSON do package", e);
        }
    }

    /**
     * Parseia metadados completos diretamente de uma String JSON.
     */
    public static PackageMetadata parseMetadata(String jsonContent) {
        try {
            return OBJECT_MAPPER.readValue(jsonContent, PackageMetadata.class);
        } catch (IOException e) {
            throw new RuntimeException("Erro ao parsear JSON do package", e);
        }
    }
    ...
    private static boolean isPackageJson(TarArchiveEntry entry) {
        return entry.isFile()
                && entry.getName().equals("package/package.json");
    }

    /**
     * Informações básicas do package (name/id e version).
     */
